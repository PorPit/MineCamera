...
public class GuiPhotoProcessor extends GuiContainer {
		...
		this.buttonList.add(new GuiButton(BUTTON_START, offsetX + 11, offsetY + 101, 25, 16, "") {
			...
			public void mouseReleased(int mouseX, int mouseY) {
				...
				if (!container.getSlot(0).getHasStack()) {
					...
					info = "§c" + I18n.format("container.photoprocessor.text.noneda");
					return;
				} else if (!container.getSlot(2).getHasStack()) {
					info = "§c" + I18n.format("container.photoprocessor.text.nonefilm");
					return;
				} else if (!container.getSlot(3).getHasStack()) {
					info = "§c" + I18n.format("container.photoprocessor.text.nonepaper");
					return;
				} else if (container.getSlot(4).getHasStack()) {
					info = "§c" + I18n.format("container.photoprocessor.text.hasphotoout");
					return;
	...
	protected void drawGuiContainerForegroundLayer(int mouseX, int mouseY) {
		...
		if (burnTime >= TileEntityPhotoProcessor.totalburnTime) {
			...
			info = "§2" + I18n.format("container.photoprocessor.text.outing");
			...
			if (textureWidth == 24) {
				info = "§2" + I18n.format("container.photoprocessor.text.success");
			}
		}
		// speed
		...
		if (container.getSlot(2).getStack() != null) {
			...
			if (PictureFactory.loadedPicture.containsKey(imagename)) {
				...
				if (imagename.split("%_%").length == 2) {
					...
					if (infolock == false) {
						info = "§2" + I18n.format("container.photoprocessor.text.readsuccess")
								+ imagename.split("%_%")[0];
					}
				}
